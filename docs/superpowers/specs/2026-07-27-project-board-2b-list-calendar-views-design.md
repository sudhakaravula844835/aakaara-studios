...
**Sorting:** clicking a column header sorts `currentProjects` client-side (comparator per column — string comparison for Client Name/Package Tier, `compareProjectsByDate` for Date, `stageIndex` for Stage, `stageIndex` again for Progress since they're equivalent), toggling direction on repeat clicks of the same header.

**Correction from final review:** the original design called for sort state to reset on every view switch (not persist). The shipped implementation persists it instead (module-level `sortState` in `list-view.js`, never reset), which the final review flagged as a spec deviation — but on reflection this is the better behavior: a PM who sorts by date, checks Kanban, then comes back to List almost certainly wants to still see it sorted by date, not lose that on every glance away. Updating the spec to match rather than reverting the code. The header must visually indicate the active sort column and direction (a caret + `aria-sort`) precisely because the state now persists silently — an invisible-but-sticky sort is worse than a visible one, which is why this was raised together with the header-indicator fix in the same review round.
