...
export function FocusRail({
  ...
  return (
    <div
      className={cn(
        "group relative flex h-[640px] w-full select-none flex-col overflow-hidden overflow-x-hidden bg-neutral-950 text-white outline-none",
        className
      )}
    ...
    >
      ...
      <div className="relative z-10 flex flex-1 flex-col justify-center px-4 md:px-8">
        {/* Draggable Rail */}
        <motion.div
          className="relative mx-auto flex h-[380px] w-full max-w-6xl cursor-grab items-center justify-center active:cursor-grabbing"
          ...
          {visibleIndices.map((offset) => {
            ...
            return (
              <motion.div
                key={absIndex}
                className={cn(
                  "absolute aspect-video w-[480px] rounded-2xl border-t border-white/20 bg-neutral-900 shadow-2xl transition-shadow duration-300 md:w-[640px]",
        ...
        <div className="pointer-events-auto mx-auto mt-12 flex w-full max-w-4xl flex-col items-center justify-between gap-6 md:flex-row">
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-1 rounded-full bg-neutral-900/80 p-1 ring-1 ring-white/10 [backdrop-filter:blur(12px)]">
              <button
                onClick={handlePrev}
            ...
            {activeItem.href && (
              <Link
                ...
                className="group/link flex cursor-pointer items-center gap-2 rounded-full bg-white px-5 py-3 text-sm font-semibold text-black transition-transform hover:scale-105 active:scale-95"
              >
                Explore
