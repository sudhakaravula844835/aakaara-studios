...
// Show/hide "+ Add Day" based on event type; non-weddings are single-day only
function updateDayMode() {
  const isWedding = document.getElementById('eventType').value === 'Wedding Photography';
  const addDayBtn = document.getElementById('addDayBtn');
  addDayBtn.style.display = isWedding ? '' : 'none';

  if (!isWedding) {
    // Remove all extra days, keep only Day 1
    const blocks = document.querySelectorAll('#daysContainer .day-block');
    blocks.forEach((block, i) => { if (i > 0) block.remove(); });
    renumberDays();
    // Also hide the "Remove" button on Day 1 since there's only one
    const removeBtn = document.querySelector('#daysContainer .day-block .day-remove');
    if (removeBtn) removeBtn.style.display = 'none';
  } else {
    // Show remove button on all days when switching back to wedding
    document.querySelectorAll('#daysContainer .day-block .day-remove').forEach(btn => {
      btn.style.display = '';
    });
  }
}

document.getElementById('eventType').addEventListener('change', updateDayMode);
