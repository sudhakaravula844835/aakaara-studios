...
# Import the image optimization function from the same build/ folder
...
def build():
    ...
    try:
        header_content = (SRC_DIR / 'templates' / 'header.html').read_text(encoding='utf-8')
        footer_content = (SRC_DIR / 'templates' / 'footer.html').read_text(encoding='utf-8')
    ...
    total_css_saved = 0
    total_js_saved = 0
    excluded_dirs = {'.git', '__pycache__', '.vscode', 'dist', 'build', 'templates', 'tools', 'docs', '.claude', '.venv', '.github'}
